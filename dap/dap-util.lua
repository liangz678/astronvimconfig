---@diagnostic disable: param-type-mismatch, undefined-field, need-check-nil
local M = {}
...
function M.store_breakpoints()
  if not utils.exists(bp_base_dir) then os.execute("mkdir -p " .. bp_base_dir) end

  -- save current buffer breakpoints
  local bps = {}
  local breakpoints_by_buf = breakpoints.get()
  for buf, buf_bps in pairs(breakpoints_by_buf) do
    if buf == vim.api.nvim_buf_get_number(0) then bps[vim.api.nvim_buf_get_name(buf)] = buf_bps end
  end
  ...
  if json_str ~= nil then fp:write(json_str) end
  fp:close()
end
